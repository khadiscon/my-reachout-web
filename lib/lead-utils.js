...
export function isPlaceholderLead(lead = {}) {
  ...
  const handles = [lead.instagram_handle, lead.x_handle].map((value) => String(value || "").toLowerCase());

  return (
    String(lead.source || "").toLowerCase() === "demo" ||
    String(lead.id || "").startsWith("demo-") ||
    email.endsWith(".example") ||
    website.includes(".example") ||
    ["founders lab podcast", "forge fitness austin", "maya ops daily"].includes(name) ||
    handles.some((handle) => ["founderslabpod", "forgefitnessatx", "mayaopsdaily"].includes(handle))
  );
}

export function productionLeads(leads = []) {
  return leads.filter((lead) => !isPlaceholderLead(lead));
}
